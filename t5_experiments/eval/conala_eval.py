...
def check_add_multiply(r):
    ...
    for i in range(len(r)):
        if r[i] == '#':
            count+=1 
        if r[i] == 'add' or r[i] == 'multiply':
            new_r = r[:]
            new_r = swap(new_r, i, count)
            combination.append(new_r)
            
    return combination

def swap(r, index, count):
    if count == 1: 
        num_x = r[index+2]
        num_y = r[index+4]
        r[index+2] = num_y
...
def calculate_bleu_from_lists(gold_texts, predicted_texts):
    ...
    for h, r  in zip(c_hyp, c_ref):
        if 'multiply' in r or 'add' in r:
            combination = check_add_multiply(r)
            if h in combination: 
                exact +=1
        else: 
            if h == r:
                exact +=1
    exact = exact/len(c_hyp)
    return bleu, exact
